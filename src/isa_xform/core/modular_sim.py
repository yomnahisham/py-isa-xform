...
class Simulator:
    def __init__(self, isa_definition: ISADefinition, symbol_table: Optional[SymbolTable] = None, disassembler: Optional[Disassembler] = None):
        ...
        self.regs = [0] * self.isa_definition.register_count  # Initialize registers
    ...
    def unsigned(self, value: int) -> int:
        """Returns the unsigned value of a given integer"""
        return value + (1 << self.isa_definition.word_size) if value < 0 else value
    
    def signed(self, value: int) -> int:
        """Returns the signed value of a given integer"""
        return value - (1 << self.isa_definition.word_size) if value >= (1 << (self.isa_definition.word_size - 1)) else value
    ...
    def execute_instruction(self, disassembled_instruction: DisassembledInstruction) -> bool:
        ...
        if disassembled_instruction.instruction:
            if disassembled_instruction.instruction.mnemonic == "ECALL":
                ...
                exec(executable_string, {'regs': self.regs, 'memory': self.memory, 'self': self, 'unsigned': self.unsigned, 'signed': self.signed})
                return True
        return True
